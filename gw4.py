...
def do_GK(fi, lam, lam0):
    ...
    # lam0 = num.deg2rad(lam0)
    # t = num.tan(fi)
    # N = a / num.sqrt(1 - e2 * num.sin(fi) ** 2)
    # eta2 = e2_prim * (num.cos(fi) ** 2)
    # delta_lam = lam - lam0
    ...
    sigma = a * (A0 * fi - A2 * num.sin(2 * fi) + A4 * num.sin(4 * fi) - A6 * num.sin(6 * fi))

    x = sigma + pow(delta_lam, 2) / 2 * N * num.sin(fi) * num.cos(fi) * (1 + pow(delta_lam, 2) / 12 * (pow(num.cos(fi), 2)) * (5 -
        pow(t, 2) + 9 * eta2 + 4 * pow(eta2, 2)) + pow(delta_lam, 4) / 360 * pow(num.cos(fi), 4) * (61 - 58 *pow(t, 2) + pow(t, 4) +
        270 * eta2 - 330 * eta2 * pow(t, 2)))
...
def z_GK(x, y, lam0):
    ...
    while True:
        ...
        sigma = a * (A0 * fi2 - A2 * num.sin(2 * fi2) + A4 * num.sin(4 * fi2) - A6 * num.sin(6 * fi2))
        N = a / pow((1 - e2 * pow(num.sin(fi2), 2)), 0.5)
        M = (a * (1 - e2)) / pow(pow((pow((1 - e2 * num.sin(fi2)), 2)), 3), 0.5)
        t = num.tan(fi2)
    ...
    fi = fi2 - pow(y, 2) * t / (2 * M * N) * (1 - pow(y, 2) / (12 * pow(N, 2)) * (5 + 3 * pow(t, 2) + eta2 - 9 * eta2 *
                                                                         pow(t, 2) - 4 * pow(eta2, 2)) + pow(y, 4) / (
                                                       360 * pow(N, 4)) * (61 + 90 * pow(t, 2) + 45 * pow(t, 4)))

    lam = lam0 + y / (N * num.cos(fi2) * (1 - pow(y, 2) / (6 * pow(N, 2)) * (1 + 2 * pow(t, 2) + eta2) + pow(y, 4) / (120 * pow(N, 4))
                                         * (5 + 28 * pow(t, 2) + 24 * pow(t, 4) + 6 * eta2 + 8 * eta2 * pow(t, 2))))
...
def skala_2000(x_gk, y_gk):
    ...
    m_gk = 1 + pow(y, 2) / (2 * pow(Q, 2)) + pow(y, 2) / (24 * pow(Q, 4))
    m2000 = m_20 * m_gk
    kappa = (1 - m2000) * 1000
...
def skala_GK(x_gk, y_gk):
    ...
    N = a / pow((1 - e2 * pow(num.sin(fi), 2)), 0.5)
    Q = num.sqrt(M * N)
    m_gk = 1 + pow(y_gk, 2) / (2 * pow(Q, 2)) + pow(y_gk, 2) / (24 * pow(Q, 4))
    kappa = (1 - m_gk) * 1000
    return m_gk, kappa
...
print("---------------------tabela-1--------------------")
print("_____________________GK_____________________")
...
print('\n')
print("_____________________92_____________________")
...
print('\n')
print("_____________________2000_____________________")
...
print('\n')
print("_____________________tabela 2_____________________")
...
print('\n')
print('\n')
print("_________________tabela 3 i 4_________________")
print("_____________________GK_____________________")
print("A: " + str(GK_m_A))
print("B: " + str(GK_m_B))
print("C: " + str(GK_m_C))
...
print('\n')
print('\n')
print("_________________tabela dodadkowa_________________")
print("________________________GK________________________")
...
print('\n')
print("_____________________92_________________________")
